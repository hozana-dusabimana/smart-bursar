...
import { Shield, Users, Settings, ChevronLeft, ChevronRight, LogOut, Activity, GraduationCap, Bell, X, Menu, Receipt } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  useEffect(() => {
    ...
    api.get('/settings/setup-status').then(r => {
      ...
      <aside className={`flex flex-col bg-gray-900 text-white shrink-0 h-screen fixed lg:relative z-40 sidebar-transition ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${collapsed ? 'lg:w-[72px]' : 'w-[240px]'}`}>
        ...
        <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
          ...
          {[
            ...
            { path: '/admin/invoices', label: 'Fees & Invoices', icon: Receipt, roles: ['admin'] },
            { path: '/admin/notifications', label: 'Notifications', icon: Bell, roles: ['admin', 'accountant'] },
