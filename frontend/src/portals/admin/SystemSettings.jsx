...
import { Loader2, CheckCircle2, Save, School, DollarSign, BookOpen } from 'lucide-react';
import api from '../../api/client';
import { fmt } from '../../utils/format';

const TABS = [
  { id: 'settings', label: 'School Information', icon: School     },
...
export default function SystemSettings() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Tab bar */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-3 py-2.5 flex flex-wrap gap-2">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all
              ${tab === t.id ? 'bg-orange-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}>
